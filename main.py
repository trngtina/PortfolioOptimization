...
with st.sidebar:
  ...
  # Integer input
  time_steps = st.number_input('Enter number of time steps:', min_value=20, max_value=150, value=50)

# App layout
st.title('Interactive Time-Varying Graph')

options = st.multiselect(label = 'Choose your tickers', options = tickers)

results = optimization.optimal_portfolio(options, hold_days=time_steps)[::-1]
