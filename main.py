...
with st.sidebar:
  ...
  minimum = st.slider("Minimum investment in asset", min_value=0.0, max_value=1.0, value = 0.0, step = 0.05)
  maximum = st.slider("Maximum investment in asset", min_value=0.0, max_value=1.0, value =0.4, step = 0.05)
...
if not options:
    ...
    color_scale = pcolors.qualitative.Plotly  # Using the 'Plotly' qualitative color scale
    ...
    for i in range(len(results)):
        fig.add_trace(
            go.Pie(
                ...
                values=results.iloc[i],
                marker=dict(colors=colors), # Assign colors to the pie chart
                sort=False,
                hole=.4,
                textinfo='label',
